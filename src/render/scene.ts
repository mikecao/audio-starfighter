...
export function setupScene(container: HTMLElement): RenderScene {
	...
	const enemyRenderStyles: Record<
		SimulationSnapshot["enemies"][number]["archetype"],
		EnemyRenderStyle
	> = {
		redCube: {
	...
	return {
		update(snapshot) {
			...
			for (let i = 0; i < enemyMeshes.length; i += 1) {
				...
				const style =
					enemyRenderStyles[enemy.archetype] ?? enemyRenderStyles.redCube;
				const material = mesh.material as MeshStandardMaterial;
				const expectedGeometry =
					enemy.archetype === "greenTriangle"
						? greenTriangleGeometry
						: redCubeGeometry;
